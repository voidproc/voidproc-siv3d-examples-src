...
void Main()
{
	...
	//Main_intersect(); // intersect.cpp : 異なる種類の図形に対し一括であたり判定をチェックする

	// ★ 画像を描く ( examples/texture/ )
	//Main_emoji_sparkle(); // emoji_sparkle.cpp : 絵文字のみを使ってキラッとするアニメーション

	// ★ 日付と時刻、時間 ( examples/time/ )
	//Main_customclock(); // customclock.cpp : Stopwatchを一括して停止・再開したり、遅く／速く進行させる（ISteadyClockの実装例）

	// ★ フォントを使う ( examples/font/ )
	//Main_crispy_font(); // crispy_font.cpp : ピクセルフォントを使用する
	//Main_draw_glyphs(); // draw_glyphs.cpp : 絵文字を含むテキストを文字単位で自由描画する
	//Main_glyph(); // glyph.cpp : グリフアニメーション

	// ★ シーンとウィンドウ ( examples/scene/ )
	//Main_low_resolution_scene(); // low_resolution_scene.cpp : 低解像度のシーンを整数倍に拡大して表示
	...
	//Main_transform(); // transform.cpp : 2D 描画位置を自由にずらす
	//Main_window_appears(); // window_appears.cpp : 画面外からぬるっと現れるビューポート

	// ★ エフェクト ( examples/effect/ )
	//Main_click_effect(); // click_effect.cpp : マウスクリック位置にエフェクト
	...
	//Main_retro(); // retro.cpp : レンダーテクスチャとガウスぼかし、ピクセルシェーダを使ったレトロコンソール的表現

	// ★ 2D カスタムシェーダ ( examples/shader2d/ )
	//Main_pixelshader_white(); // pixelshader_white.cpp : テクスチャを白く点滅させるピクセルシェーダ

	// ★ 3D 形状を描く ( examples/3d/ )
	//Main_dice(); // dice.cpp : サイコロを描く

	// ★ 未分類 ( examples/misc/ )
	//Main_decodeimage(); // decodeimage.cpp : Base64 でエンコードした画像をソースコードに埋め込み、表示
