...
void Main()
{
	...
	// 基準時刻
	std::shared_ptr<CustomClock> clock = std::make_shared<CustomClock>(StartImmediately::Yes);

	// これらを一括で停止したり再開したり、速度を変えたりしたい
	Stopwatch stopwatch1{ StartImmediately::Yes, clock.get() };
	VariableSpeedStopwatch stopwatch2{ 0.3, StartImmediately::Yes, clock.get() };
	VariableSpeedStopwatch stopwatch3{ 1.5, StartImmediately::Yes, clock.get() };
